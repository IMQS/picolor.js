...
(function(l) {
  ...
  l.whiteToBlackInterpolator = chroma.interpolate.bezier(["white", "black"]);
  var r = function() {
    function c(a, b) {
      var m = this;
      ...
      $("#" + this.containerDivID).on("mousedown", "#" + this.colorWheelDivID, function(a) {
        m.onColorWheelMouseDown(a);
        m.onColorWheelMouseMove(a);
      });
    ...
    c.prototype.onColorWheelMouseDown = function(a) {
      var b = a.pageX - this.offset.left;
      a = a.pageY - this.offset.top;
      var m = b - this.cx, c = a - this.cy;
      Math.sqrt(m * m + c * c) < this.radius && (this.isDraggingColor = !0);
    ...
    c.prototype.onColorWheelMouseMove = function(a) {
      if (this.isDraggingAlpha || this.isDraggingColor || this.isDraggingLightness) {
        var b = a.pageX - this.offset.left;
        a = a.pageY - this.offset.top;
        if (this.isDraggingColor) {
          var m = b - this.cx, c = a - this.cy, b = Math.min(this.radius, Math.sqrt(m * m + c * c)), m = 180 * Math.atan2(c, m) / Math.PI;
          this.lch = [this.lch[0], 100 * b / this.radius, m];
        }
    ...
    c.prototype.onColorWheelMouseUp = function(a) {
    ...
    c.prototype.drawContainerStructure = function() {
      var a = '<div class="picolor-container">\t<div class="picolor-top-container">\t\t<div id="' + this.colorBandDivID + '-0"></div>\t\t<div id="' + this.colorBandDivID + '-1"></div>\t\t<div id="' + this.blackToWhiteBandDivID + '" style="margin-top: 6px"></div>\t</div>\t<div class="picolor-bottom-container">\t\t<canvas id="' + this.colorWheelDivID + '" class="picolor-wheel"></canvas>\t</div></div>', b = $("#" + this.containerDivID);
      b.empty();
      b.append(a);
    };
    c.prototype.drawBasicSelector = function() {
      for (var a = this, b = function(b, c) {
        ...
        c.push(l.whiteToBlackInterpolator(0.2 * t));
      }
    ...
    c.prototype.drawColorWheel = function() {
      ...
      var a = document.getElementById(this.colorWheelDivID), b = a.getContext("2d"), c, l, r = Number.MAX_VALUE;
      a.width = this.width;
      a.height = this.height;
      for (var a = b.createImageData(this.width, this.height), k = a.data, d = 0, f = 0;f < this.height;f++) {
        for (var g = 0;g < this.width;g++, d += 4) {
          var h = g - this.cx, e = f - this.cy, u = Math.sqrt(h * h + e * e);
          if (u < this.radius + 0.5) {
            e = 180 * Math.atan2(e, h) / Math.PI;
            0 > e && (e += 360);
            360 < e && (e -= 360);
            var q = 100 * u / this.radius, h = chroma.lch2rgb(this.lch[0], q, e), e = Math.sqrt(Math.pow(q - this.lch[1], 2) + Math.pow(e - this.lch[2], 2));
            e < r && (r = e, c = g, l = f);
            var e = this.alpha, q = h[0], v = h[1], w = h[2], n = 0 == Math.floor(g / 5) % 2, p = 0 == Math.floor(f / 5) % 2, s = n && !p || !n && p ? 250 : 200, p = n = s;
            k[d] = q * e / 255 + n * (1 - e / 255);
            k[d + 1] = v * e / 255 + p * (1 - e / 255);
            k[d + 2] = w * e / 255 + s * (1 - e / 255);
            k[d + 3] = 255 * Math.max(0, this.radius - u);
          }
        }
      }
      for (f = d = 0;f < this.height;f++) {
        for (g = 0;g < this.width;g++, d += 4) {
          30 > f || f > this.height - 30 || 2 > g || 15 < g || (h = chroma.lch2rgb(100 - 100 * (f - 30) / (this.height - 60), this.lch[1], this.lch[2]), k[d] = h[0], k[d + 1] = h[1], k[d + 2] = h[2], k[d + 3] = 255);
        }
      }
      d = 0;
      ...
      for (f = 0;f < this.height;f++) {
        for (g = 0;g < this.width;g++, d += 4) {
          30 > f || f > this.height - 30 || g < this.width - 16 || g > this.width - 3 || (e = 255 - 255 * (f - 30) / (this.height - 60), q = h[0], v = h[1], w = h[2], n = 0 == Math.floor(g / 5) % 2, p = 0 == Math.floor(f / 5) % 2, p = n = s = n && !p || !n && p ? 250 : 200, k[d] = q * e / 255 + n * (1 - e / 255), k[d + 1] = v * e / 255 + p * (1 - e / 255), k[d + 2] = w * e / 255 + s * (1 - e / 255), k[d + 3] = 255);
        }
      }
