...
(function(m) {
  ...
  var p = function() {
    ...
    Object.defineProperty(f.prototype, "lch", {get:function() {
      return this._lch;
    }, set:function(a) {
      0 > a[2] && (a[2] += 360);
      360 <= a[2] && (a[2] -= 360);
    ...
    Object.defineProperty(f.prototype, "color", {get:function() {
    ...
    f.prototype.draw = function() {
      ...
      for (var b = function(b, c) {
        for (var e = "", d = 0;d < b.length;d++) {
          var f = c + "-" + d, e = e + ('<div id="' + f + '" style="width: 38px;height: 24px;cursor: pointer;display: inline-block;padding: 1px;background-color: #E0E0E0;margin: 0px 4px 0px 4px;'), e = a.color.css() === b[d].css() ? e + " border: 2px solid black;" : e + " border: 2px solid #E0E0E0;", e = e + '">', e = e + ('\t<div style="height: 24px; background-color:' + b[d].css() + '"></div>'), e = e + "</div>";
          $("#" + a.containerDivID).on("click", "#" + f, b[d], function(b) {
            a.lch = b.data.lch();
        ...
        c.push(m.whiteToBlackInterpolator(0.2 * d));
      }
  ...
  p = function() {
    function f(a, b) {
      ...
      var d = '<div style="background-color: #E0E0E0; padding 5px">\t<canvas id="' + this.colorWheelDivID + '"></canvas></div>', f = $("#" + this.containerDivID);
      f.empty();
      f.append(d);
    ...
    f.prototype.setWheelDragStateOn = function(a) {
    ...
    f.prototype.setCursor = function(a) {
      if (!(this.isDraggingAlpha || this.isDraggingColor || this.isDraggingLightness)) {
        var b = a.pageX - this.offset.left;
        a = a.pageY - this.offset.top;
        var c = b - this.cx, d = a - this.cy;
        Math.sqrt(c * c + d * d) < this.radius ? $("#" + this.colorWheelDivID).css("cursor", "crosshair") : 30 <= a && a <= this.height - 30 && -2 <= b && 18 >= b ? $("#" + this.colorWheelDivID).css("cursor", "s-resize") : 30 <= a && a <= this.height - 30 && b >= this.width - 19 && b <= this.width + 1 ? $("#" + this.colorWheelDivID).css("cursor", "s-resize") : $("#" + this.colorWheelDivID).css("cursor", "default");
      }
    };
    f.prototype.setWheelColor = function(a) {
      if (this.isDraggingAlpha || this.isDraggingColor || this.isDraggingLightness) {
        var b = a.pageX - this.offset.left;
        a = a.pageY - this.offset.top;
        if (this.isDraggingColor) {
          var c = b - this.cx, d = a - this.cy, b = Math.min(this.radius, Math.sqrt(c * c + d * d)), c = 180 * Math.atan2(d, c) / Math.PI;
    ...
    f.prototype.setWheelDragStateOff = function(a) {
    ...
    Object.defineProperty(f.prototype, "alpha", {get:function() {
      return this._alpha;
    }, set:function(a) {
      this._alpha = a;
      this.draw();
    ...
    f.prototype.draw = function() {
      ...
      var a = document.getElementById(this.colorWheelDivID), b = a.getContext("2d"), c, d, f = Number.MAX_VALUE;
      a.width = this.width;
      a.height = this.height;
      for (var a = b.createImageData(this.width, this.height), g = a.data, e = 0, k = 0;k < this.height;k++) {
        for (var l = 0;l < this.width;l++, e += 4) {
          ...
          if (m < this.radius + 0.5) {
            ...
            g[e + 1] = p * h + r * (1 - h);
            g[e + 2] = u * h + t * (1 - h);
            g[e + 3] = 255 * Math.max(0, this.radius - m);
          }
        }
      }
      for (k = e = 0;k < this.height;k++) {
        for (l = 0;l < this.width;l++, e += 4) {
          30 > k || k > this.height - 30 || 2 > l || 15 < l || (n = chroma.lch2rgb(100 - 100 * (k - 30) / (this.height - 60), this.lch[1], this.lch[2]), g[e] = n[0], g[e + 1] = n[1], g[e + 2] = n[2], g[e + 3] = 255);
        }
      }
      e = 0;
      ...
      for (k = 0;k < this.height;k++) {
        for (l = 0;l < this.width;l++, e += 4) {
          30 > k || k > this.height - 30 || l < this.width - 16 || l > this.width - 3 || (h = 1 - (k - 30) / (this.height - 60), s = n[0], p = n[1], u = n[2], q = 0 == Math.floor(l / 5) % 2, r = 0 == Math.floor(k / 5) % 2, r = q = t = q && !r || !q && r ? 250 : 200, g[e] = s * h + q * (1 - h), g[e + 1] = p * h + r * (1 - h), g[e + 2] = u * h + t * (1 - h), g[e + 3] = 255);
        }
      }
      ...
      f = 30 + (100 - this.lch[0]) / 100 * (this.height - 60);
      f = Math.max(37, Math.min(this.height - 30 - 6, f));
      b.arc(9, f, 5, 0, 2 * Math.PI, !1);
      b.stroke();
      b.beginPath();
      0.5 > this.alpha && (b.strokeStyle = "#2f2f2f");
      b.arc(c, d, 6, 0, 2 * Math.PI, !1);
      b.stroke();
      b.beginPath();
  ...
  p = function() {
    ...
    Object.defineProperty(f.prototype, "hexPalette", {get:function() {
      for (var a = [], b = this.palMatrix[this.selectedPalIdx], c = 0;c < b.length;c++) {
        a.push(b[c].hex());
      }
      return a;
    }, enumerable:!0, configurable:!0});
    Object.defineProperty(f.prototype, "categoryCount", {get:function() {
    ...
    f.prototype.draw = function() {
      ...
      var a = document.getElementById(this.paletteCanvasDivID), b = a.getContext("2d"), c = this.categoryCount, d = this.sequentialPalettes.length + this.divergentPalettes.length + this.qualitativePalettes.length;
      a.width = 2 * this.margin + this.w * d + this.pad * (d - 1);
      a.height = 2 * this.margin + this.h * c;
      for (a = 0;a < this.sequentialPalettes.length;a++) {
        for (var d = [], f = this.sequentialPalettes[a], g = 0;g < c;g++) {
          var e = g / (c - 1);
          d.push(f(e));
        }
      ...
      for (a = 0;a < this.divergentPalettes.length;a++) {
        for (var d = [], f = this.divergentPalettes[a][0], k = this.divergentPalettes[a][1], g = 0;g < c;g++) {
          e = g / (c - 1), e = 0.5 >= e ? f(2 * e) : k(2 * (e - 0.5)), d.push(e);
        }
        this.palMatrix.push(d);
      }
      for (a = 0;a < this.qualitativePalettes.length;a++) {
        d = [];
        e = this.qualitativePalettes[a];
        for (g = 0;g < e.length && g < c;g++) {
          d.push(chroma.hex(e[g]));
        }
        this.palMatrix.push(d);
      }
      ...
      b.strokeStyle = "black";
      b.lineWidth = 2;
      b.strokeRect(this.margin - 2 + this.selectedPalIdx * (this.w + this.pad), this.margin - 2, this.w + 4, this.h * c + 4);
    };
    return f;
  }();
  m.Palette = p;
