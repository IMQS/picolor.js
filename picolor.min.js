...
(function(m) {
  ...
  var p = function() {
    function e(a, b) {
      ...
      var f = '<div class="picolor-container">\t<div id="' + this.colorBandDivID + '-0"></div>\t<div id="' + this.colorBandDivID + '-1"></div>\t<div id="' + this.blackToWhiteBandDivID + '" style="margin-top: 6px"></div></div>', c = $("#" + this.containerDivID);
      c.empty();
      c.append(f);
    }
    ...
    e.prototype.draw = function() {
      ...
      for (var b = function(b, f) {
        for (var d = "", c = 0;c < b.length;c++) {
          var e = f + "-" + c, d = d + ('<div id="' + e + '" class="picolor-box-container');
          a.color.css() === b[c].css() && (d += " picolor-box-container-selected");
          d += '">';
          d += '\t<div class="picolor-box" style="background-color:' + b[c].css() + '"></div>';
          d += "</div>";
          $("#" + a.containerDivID).on("click", "#" + e, b[c], function(b) {
            a.color = b.data;
        ...
        f.push(m.whiteToBlackInterpolator(0.2 * c));
      }
  ...
  p = function() {
    function e(a, b) {
      ...
      var c = '<div class="picolor-container">\t<canvas id="' + this.colorWheelDivID + '"></canvas></div>', e = $("#" + this.containerDivID);
      e.empty();
      e.append(c);
    ...
    e.prototype.setWheelDragStateOn = function(a) {
      ...
      var f = b - this.cx, c = a - this.cy;
      Math.sqrt(f * f + c * c) < this.radius && (this.isDraggingColor = !0);
    ...
    e.prototype.setWheelColor = function(a) {
      if (this.isDraggingAlpha || this.isDraggingColor || this.isDraggingLightness) {
        ...
        if (this.isDraggingColor) {
          var f = b - this.cx, c = a - this.cy, b = Math.min(this.radius, Math.sqrt(f * f + c * c)), f = 180 * Math.atan2(c, f) / Math.PI;
        ...
        this.isDraggingAlpha && (this.alpha = Math.max(0, Math.min(1, 1 - 1 * (a - 30) / (this.height - 60))));
      }
    };
    ...
    e.prototype.draw = function() {
      ...
      var a = document.getElementById(this.colorWheelDivID), b = a.getContext("2d"), f, c, e = Number.MAX_VALUE;
      a.width = this.width;
      a.height = this.height;
      for (var a = b.createImageData(this.width, this.height), g = a.data, d = 0, k = 0;k < this.height;k++) {
        for (var l = 0;l < this.width;l++, d += 4) {
          ...
          if (m < this.radius + 0.5) {
            ...
            h < e && (e = h, f = l, c = k);
            var h = 255 * this.alpha, s = n[0], p = n[1], u = n[2], q = 0 == Math.floor(l / 5) % 2, r = 0 == Math.floor(k / 5) % 2, t = q && !r || !q && r ? 250 : 200, r = q = t;
            g[d] = s * h / 255 + q * (1 - h / 255);
            g[d + 1] = p * h / 255 + r * (1 - h / 255);
            g[d + 2] = u * h / 255 + t * (1 - h / 255);
            g[d + 3] = 255 * Math.max(0, this.radius - m);
          }
        }
      }
      for (k = d = 0;k < this.height;k++) {
        for (l = 0;l < this.width;l++, d += 4) {
          30 > k || k > this.height - 30 || 2 > l || 15 < l || (n = chroma.lch2rgb(100 - 100 * (k - 30) / (this.height - 60), this.lch[1], this.lch[2]), g[d] = n[0], g[d + 1] = n[1], g[d + 2] = n[2], g[d + 3] = 255);
        }
      }
      d = 0;
      ...
      for (k = 0;k < this.height;k++) {
        for (l = 0;l < this.width;l++, d += 4) {
          30 > k || k > this.height - 30 || l < this.width - 16 || l > this.width - 3 || (h = 255 - 255 * (k - 30) / (this.height - 60), s = n[0], p = n[1], u = n[2], q = 0 == Math.floor(l / 5) % 2, r = 0 == Math.floor(k / 5) % 2, r = q = t = q && !r || !q && r ? 250 : 200, g[d] = s * h / 255 + q * (1 - h / 255), g[d + 1] = p * h / 255 + r * (1 - h / 255), g[d + 2] = u * h / 255 + t * (1 - h / 255), g[d + 3] = 255);
        }
      }
  ...
  p = function() {
    function e(a, b) {
      ...
      b && this.setOptions(b);
      var f = '<div class="picolor-container">\t<canvas id="' + this.paletteCanvasDivID + '"></canvas></div>', c = $("#" + this.containerDivID);
      c.empty();
      c.append(f);
    ...
    e.prototype.draw = function() {
      ...
      var a = document.getElementById(this.paletteCanvasDivID), b = a.getContext("2d"), f = this.categoryCount, c = this.sequentialPalettes.length + this.divergentPalettes.length + this.qualitativePalettes.length;
      a.width = 2 * this.margin + this.w * c + this.pad * (c - 1);
      a.height = 2 * this.margin + this.h * f;
      for (a = 0;a < this.sequentialPalettes.length;a++) {
        for (var c = [], e = this.sequentialPalettes[a], g = 0;g < f;g++) {
          var d = g / (f - 1);
          c.push(e(d));
        }
        this.palMatrix.push(c);
      }
      for (a = 0;a < this.divergentPalettes.length;a++) {
        for (var c = [], e = this.divergentPalettes[a][0], k = this.divergentPalettes[a][1], g = 0;g < f;g++) {
          d = g / (f - 1), d = 0.5 >= d ? e(2 * d) : k(2 * (d - 0.5)), c.push(d);
        }
        this.palMatrix.push(c);
      }
      for (a = 0;a < this.qualitativePalettes.length;a++) {
        c = [];
        d = this.qualitativePalettes[a];
        for (g = 0;g < d.length && g < f;g++) {
          c.push(chroma.hex(d[g]));
        }
        this.palMatrix.push(c);
      }
      for (a = 0;a < this.palMatrix.length;a++) {
        for (c = this.palMatrix[a], g = 0;g < c.length;g++) {
          b.fillStyle = c[g], b.fillRect(this.margin + a * (this.w + this.pad), this.margin + g * this.h, this.w, this.h);
        }
      }
      b.strokeStyle = "black";
      b.lineWidth = 2;
